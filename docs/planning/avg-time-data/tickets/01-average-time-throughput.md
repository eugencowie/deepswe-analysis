# 01: Average time via OpenRouter throughput
...
- The seed snapshot stores `medianP50` only. The research capture recorded no per-endpoint values, so the spec type's `endpoints` array moves to automated-refresh ticket 02, whose refresh script actually produces it — checked-in data shouldn't contain fabricated empties.
