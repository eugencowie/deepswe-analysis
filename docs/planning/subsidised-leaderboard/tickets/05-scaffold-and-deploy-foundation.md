...
Per the [spec](../spec.md) (Toolchain, Repo and deploy): Vite+ template, pnpm, mise with the vite-plus plugin; public GitHub repo `deepswe-enhanced` (no remote exists yet); Actions workflow gated by `vp run ready` that builds and publishes to Pages with the project base path.
...
Deployed: the template site is live at <https://eugencowie.github.io/deepswe-enhanced/>. `vp run ready` passes locally and gates the deploy workflow.
